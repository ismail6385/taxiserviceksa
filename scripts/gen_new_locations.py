...
def generate_location_page(city_name):
    slug = city_name.lower().replace(' ', '-').replace("'", "")
    component_name = city_name.replace(' ', '').replace("'", "").replace("-", "")
    
    img1 = random.randint(1, 5)
    img2 = random.randint(1, 5)
    img3 = random.randint(1, 5)
...
export const metadata: Metadata = {{
    title: 'VIP Private Transfer & Chauffeur {city_name} | Taxi Service KSA',
    description: 'Premium VIP private transfer and executive chauffeur service in {city_name}. Professional, reliable intercity transport and airport pickups.',
    keywords: ['{city_name} VIP private transfer', 'Executive chauffeur {city_name}', '{city_name} Airport VIP pickup', 'Premium driver {city_name}'],
    alternates: {{
        canonical: 'https://taxiserviceksa.com/locations/{slug}/',
    }},
    openGraph: {{
        title: '{city_name} VIP Transfer & Pickups | Taxi Service KSA',
        description: 'Need a premium VIP transfer in {city_name}? We provide comfortable chauffeured trips and intercity transport.',
        url: 'https://taxiserviceksa.com/locations/{slug}/',
        type: 'website',
    }},
}};

export default function {component_name}LocationPage() {{
    const images = [
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <JsonLdLocation 
                cityName="{city_name}"
                description="Professional VIP transfers in {city_name}. Specializing in airport pickups, corporate travel, and comfortable intercity transfers."
                services={[
                    {{ name: '{city_name} Airport Pickup', description: 'VIP meet & greet at the airport with flight tracking.' }},
            ...
            <Hero
                images={{images}}
                h1Text="VIP Private Transfer & Chauffeur {city_name}"
                bookingFormTitle="WhatsApp Booking for Premium Transfer in {city_name}"
                title={{
                    <span className="bg-white/20 backdrop-blur-md border border-white/20 text-white font-semibold tracking-wider uppercase px-4 py-2 rounded-lg inline-block decoration-clone leading-snug">
                        VIP Chauffeur Service {city_name}
                    </span>
                }}
                subtitle="Premium Executive Transport Excellence"
                location="{city_name} | Intercity Transfers | Airport Pickups"
            >
                <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
                    <Link href="/booking?pickup={city_name}&dropoff=Selected%20Hotel">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        <div>
                            ...
                            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2 mb-6">
                                VIP Transfer Services in {city_name}
                            </h2>
                            <p className="text-lg text-gray-600 mb-6 leading-relaxed">
                                Navigating {city_name} requires a reliable and comfortable mode of transport. Whether you are traveling for business, visiting family, or heading out for a long-distance intercity journey, our premium fleet is ready to serve you.
                            </p>
                        ...
                        <div className="bg-gray-100 rounded-3xl p-8 border border-gray-200">
                            <h3 className="text-2xl font-bold text-gray-900 mb-6">Why Choose Us in {city_name}?</h3>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 mb-16">
                <RelatedLocations currentCity="{city_name}" />
            </div>

            <MicroSemanticFAQ
                faqs={[
                    {{
                        question: "How do I book a transfer from {city_name}?",
...
base_path = "d:/Taxi KSA/taxiksa/app/(main)/locations"

for city in cities:
    slug, content = generate_location_page(city)
...
print(f"Successfully generated {len(cities)} location pages in app/(main)/locations/")
