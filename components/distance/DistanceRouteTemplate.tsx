...
import { getDistanceRoute, type DistanceRoute } from '@/data/distanceRoutes';

interface DistanceRouteTemplateProps {
    route: DistanceRoute;
}

// Shared structural template for /distance/{slug}/ pages, built from the approved
// /distance/kuwait-to-makkah/ page as the structural reference. That page itself is
// NOT refactored to use this component — it's left exactly as approved. This template
// exists purely so the next N routes stay structurally identical without copy-pasting
// ~350 lines of JSX into every new page.tsx file. Only route-specific data (from
// data/distanceRoutes.ts) varies the output; the border-crossing section is included
// only when route.isInternational is true.
export default function DistanceRouteTemplate({ route }: DistanceRouteTemplateProps) {
    const isInternational = Boolean(route.isInternational && route.borderName);
    ...
    const webPageSchema = {
        ...
        url: `https://taxiserviceksa.com/distance/${route.slug}/`,
        about: {
            '@type': 'Trip',
    ...
    const relatedDistanceRoutes = (route.relatedDistanceSlugs ?? [])
        .map((slug) => getDistanceRoute(slug))
    ...
    const whatsappHref = `https://wa.me/966590209905?text=${encodeURIComponent(`Hello, I want a quote for a taxi from ${route.origin} to ${route.destination}.`)}`;
        ...
        <div className="bg-gray-50 min-h-screen">
            <JsonLdBreadcrumb
                items={[
                    ...
                    { name: route.h1.replace(' by Road', ''), item: `/distance/${route.slug}` },
                ]}
            />
            <script
                id={`${route.slug}-distance-schema`}
            ...
            <section className="bg-gray-900 text-white py-14 sm:py-20 px-4 sm:px-6 lg:px-8">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-gray-300 text-lg leading-relaxed max-w-3xl mx-auto">
                        {route.origin} to {route.destination} is {route.distanceRange} by road ({route.distanceHeadline}), with {route.drivingTimeRange}.{' '}
                        {isInternational
                            ? `The route crosses into Saudi Arabia and continues to ${route.destination}, crossing the border at ${route.borderName}.`
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <section className="py-10">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.origin} to {route.destination} Road Distance</h2>
                ...
                <section className="py-10 border-t border-gray-200">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.origin} to {route.destination} Driving Time</h2>
                ...
                {/* Section: Map */}
                <section className="py-10 border-t border-gray-200">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.origin} to {route.destination} Road Map</h2>
                    <p className="text-gray-600 text-lg leading-relaxed mb-6">
                        Use the map below for a live, general reference of the driving route between {route.origin} and {route.destination}. For turn-by-turn navigation on your actual travel date, open the route directly in Google Maps.
                    </p>
                    <DistanceRouteMap
                ...
                {/* Section: Route */}
                <section className="py-10 border-t border-gray-200">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.origin} to {route.destination} Route</h2>
                    {route.routeJourneyDescription.map((p, i) => (
                        <p key={i} className="text-gray-600 text-lg leading-relaxed mb-4">{p}</p>
                    ))}
                </section>

                {/* Section: Border Crossing — international routes only */}
                {isInternational && route.borderDescription && (
                    <section className="py-10 border-t border-gray-200">
                        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.originCountry}–Saudi Arabia Border Crossing</h2>
                ...
                {/* Section: By Car */}
                <section className="py-10 border-t border-gray-200">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-5">{route.origin} to {route.destination} by Car</h2>
                    <p className="text-gray-600 text-lg leading-relaxed mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                        <div className="bg-white rounded-2xl p-6 border border-gray-100">
                            ...
                            <p className="text-sm text-gray-500">Full control of your schedule and stops, but you manage the navigation{isInternational ? ' and your own vehicle\'s border formalities' : ' yourself'}.</p>
                        </div>
                        <div className="bg-white rounded-2xl p-6 border border-gray-100">
                            <h3 className="font-bold text-gray-900 mb-2">Private Taxi</h3>
                            <p className="text-sm text-gray-500">A pre-booked vehicle and driver for the full route. No navigation to manage, one fixed vehicle door-to-door.</p>
                        </div>
                        <div className="bg-white rounded-2xl p-6 border border-gray-100">
                    ...
                    {route.byCarNote && (
                        <p className="text-gray-600 text-lg leading-relaxed">{route.byCarNote}</p>
                    )}
                </section>
                ...
                <section className="py-10 border-t border-gray-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[
                            isInternational
                                ...
                                : { icon: FileText, text: 'Valid ID documents for every passenger' },
                            { icon: Droplets, text: 'Water and snacks for the drive' },
                            ...
                            { icon: Sun, text: 'Planned rest breaks appropriate to the length of this route' },
                    ...
                    {route.journeyPlanningNote && (
                        <p className="text-gray-600 text-base leading-relaxed mt-6">{route.journeyPlanningNote}</p>
                    )}
                </section>
                ...
                <section className="py-10 border-t border-gray-200">
                    ...
                    <p className="text-gray-600 text-lg leading-relaxed mb-4">
                        There&apos;s no single &quot;best&quot; departure time we can guarantee will beat traffic{isInternational ? ' or border queues' : ''} — both vary by day and by season. What we can say generally:
                    </p>
                    <ul className="space-y-3 mb-4">
                        <li className="flex items-start gap-3">
                            <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                            <span className="text-gray-700">Road traffic varies by time of day and day of the week, same as any route.</span>
                        </li>
                        {isInternational && (
                            <li className="flex items-start gap-3">
                                <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                                <span className="text-gray-700">Border traffic at {route.borderName} varies by day and volume, and can be busier around weekends and holidays.</span>
                            </li>
                        )}
                        <li className="flex items-start gap-3">
                            <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                            <span className="text-gray-700">Hajj and Umrah peak seasons bring significantly more traffic on approaches to {route.destination} — build in extra time if traveling during these periods.</span>
                        </li>
                        <li className="flex items-start gap-3">
                            <CheckCircle2 className="w-5 h-5 text-emerald-600 mt-0.5 shrink-0" />
                            <span className="text-gray-700">If you have a fixed appointment, flight, or check-in time in {route.destination}, build in a meaningful buffer rather than cutting it close.</span>
                        </li>
                    </ul>
                    {route.bestTimeNote && (
                        <p className="text-gray-600 text-base leading-relaxed">{route.bestTimeNote}</p>
                    )}
                </section>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
                ...
                <MicroSemanticFAQ contextName={`${route.origin} to ${route.destination} Distance`} faqs={faqItems} />
            </div>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-gray-200 pt-16 mt-4 pb-8">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-500 mt-2">More GCC-to-Saudi road distance guides</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {relatedDistanceRoutes.map((r) => (
                        <Link
                            key={r.slug}
                        ...
                        >
                            <h3 className="font-bold text-gray-900 text-sm group-hover:text-primary transition-colors mb-2">{r.h1}</h3>
                            <div className="flex items-center text-xs font-semibold text-primary">
                                {r.distanceHeadline} · {r.drivingTimeHeadline}
                            </div>
                        </Link>
                    ))}
                    <Link
                    ...
                    >
                        <span className="font-bold text-gray-700 group-hover:text-primary transition-colors text-sm">Browse All Distance Guides</span>
