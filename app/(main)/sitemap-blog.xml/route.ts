...
export async function GET() {
    ...
    // Dynamically list all guide directories (excludes the `international`
    // route group, which is a dynamic [slug] page handled separately below).
    const guidesDirectory = path.join(process.cwd(), 'app', '(main)', 'guides');
    const guides = fs.readdirSync(guidesDirectory).filter(file => {
        if (file === 'international') return false;
        const filePath = path.join(guidesDirectory, file);
        return fs.statSync(filePath).isDirectory();
    });

    // International pilgrim guides (guides/international/[slug]) — previously
    // generated by these pages but never listed in any linked sitemap, so
    // Google had no way to discover them. A couple of slugs are defined twice
    // in the source data; de-dupe so each URL appears once.
    const internationalGuideSlugs = Array.from(new Set(internationalGuides.map(g => g.slug)));
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        <!-- International Pilgrim Guides Index -->
        <url>
            <loc>${baseUrl}/guides/international/</loc>
            <lastmod>${new Date().toISOString()}</lastmod>
            <changefreq>monthly</changefreq>
            <priority>0.6</priority>
        </url>

        <!-- Individual International Pilgrim Guides -->
        ${internationalGuideSlugs.map((slug: string) => `
            <url>
                <loc>${baseUrl}/guides/international/${slug}/</loc>
                <lastmod>${new Date().toISOString()}</lastmod>
                <changefreq>monthly</changefreq>
